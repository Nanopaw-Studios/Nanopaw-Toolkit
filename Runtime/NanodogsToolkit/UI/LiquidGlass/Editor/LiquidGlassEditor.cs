    ...
    public class LiquidGlassEditor : UnityEditor.Editor
    {
        ...
        private void DrawBlurSection()
        {
            ...
            if (s_ShowBlur)
            {
                ...
                DrawGlobalBlurControls();
            }
            EditorGUILayout.EndFoldoutHeaderGroup();
        }

        private void DrawGlobalBlurControls()
        {
            UniversalRendererData rendererData = GetActiveRendererData();
            ScreenBlurRendererFeature feature = null;
            if (rendererData != null && rendererData.rendererFeatures != null)
            {
                foreach (var f in rendererData.rendererFeatures)
                {
                    if (f is ScreenBlurRendererFeature sbf)
                    {
                        feature = sbf;
                        break;
                    }
            ...
            if (feature != null && feature.Settings != null)
            {
                EditorGUILayout.Space(6);
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                EditorGUILayout.LabelField("🌐 Global Pipeline Blur (Active Renderer)", EditorStyles.boldLabel);

                EditorGUI.BeginChangeCheck();
                var settings = feature.Settings;
                var newAlgo = (BlurAlgorithm)EditorGUILayout.EnumPopup(new GUIContent("Algorithm", "Dual Kawase provides smoother multi-scale blur, Separable Gaussian provides standard 1D passes."), settings.algorithm);
                var newDown = (BlurDownsample)EditorGUILayout.EnumPopup(new GUIContent("Downsampling", "Higher downsampling (Half / Quarter) produces dramatically wider blur with high GPU performance."), settings.downsample);
                int newIters = EditorGUILayout.IntSlider(new GUIContent("Iterations (Pyramid)", "Number of multi-resolution pyramid passes. Higher values dramatically expand blur radius."), settings.iterations, 1, 8);
                float newSpread = EditorGUILayout.Slider(new GUIContent("Spread (Radius)", "Blur kernel offset multiplier across the screen."), settings.spread, 0.2f, 10f);

                if (EditorGUI.EndChangeCheck())
                {
                    Undo.RecordObject(rendererData, "Modify Screen Blur Settings");
                    settings.algorithm = newAlgo;
                    settings.downsample = newDown;
                    settings.iterations = newIters;
                    settings.spread = newSpread;
                    EditorUtility.SetDirty(rendererData);
                }

                EditorGUILayout.Space(4);
                EditorGUILayout.LabelField("Global Blur Presets:", EditorStyles.miniBoldLabel);
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("VisionOS Glass", EditorStyles.miniButtonLeft))
                {
                    Undo.RecordObject(rendererData, "VisionOS Blur Preset");
                    settings.algorithm = BlurAlgorithm.DualKawase;
                    settings.downsample = BlurDownsample.Half;
                    settings.iterations = 4;
                    settings.spread = 2.2f;
                    EditorUtility.SetDirty(rendererData);
                }
                if (GUILayout.Button("Deep Acrylic", EditorStyles.miniButtonMid))
                {
                    Undo.RecordObject(rendererData, "Deep Acrylic Blur Preset");
                    settings.algorithm = BlurAlgorithm.DualKawase;
                    settings.downsample = BlurDownsample.Quarter;
                    settings.iterations = 5;
                    settings.spread = 3.0f;
                    EditorUtility.SetDirty(rendererData);
                }
                if (GUILayout.Button("Ultra Haze", EditorStyles.miniButtonMid))
                {
                    Undo.RecordObject(rendererData, "Ultra Haze Blur Preset");
                    settings.algorithm = BlurAlgorithm.DualKawase;
                    settings.downsample = BlurDownsample.Quarter;
                    settings.iterations = 6;
                    settings.spread = 4.5f;
                    EditorUtility.SetDirty(rendererData);
                }
                if (GUILayout.Button("Subtle Glaze", EditorStyles.miniButtonRight))
                {
                    Undo.RecordObject(rendererData, "Subtle Glaze Preset");
                    settings.algorithm = BlurAlgorithm.DualKawase;
                    settings.downsample = BlurDownsample.Half;
                    settings.iterations = 2;
                    settings.spread = 1.2f;
                    EditorUtility.SetDirty(rendererData);
                }
                EditorGUILayout.EndHorizontal();
