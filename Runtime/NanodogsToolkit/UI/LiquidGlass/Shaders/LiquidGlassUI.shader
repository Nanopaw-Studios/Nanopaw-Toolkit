Shader "UI/LiquidGlass/LiquidGlassUI" {
    Properties {
        ...
        [KeywordEnum(None, Uniform, Independent)] _CornerMode ("Corner Mode", Float) = 1
        _WidthHeightRadius ("WidthHeightRadius (Uniform)", Vector) = (100, 100, 20, 0)
        _halfSize ("Half Size (Independent)", Vector) = (50, 50, 0, 0)
        _r ("Radiuses (TL, TR, BR, BL)", Vector) = (15, 15, 15, 15)
        _rect2props ("Rect2 Props (Internal)", Vector) = (0, 0, 0, 0)
        ...
        _BlurAmount ("Blur Mix", Range(0, 2)) = 1.0
        _Saturation ("Saturation Boost", Range(0, 3)) = 1.25
    ...
    SubShader {
        ...
        Pass {
            ...
            float _CornerMode; // 0: None, 1: Uniform, 2: Independent
            float4 _WidthHeightRadius; // xy: size, z: radius
            float4 _halfSize;
            float4 _r;
            float4 _rect2props;
            ...
            fixed4 frag_lg(v2f_lg i) : SV_Target {
                // 1. Calculate Signed Distance Field based on Corner Mode
                float dist = -100.0;
                float cornerAlpha = 1.0;
                float2 shapeSize = _WidthHeightRadius.xy;

                if (_CornerMode > 1.5) {
                    // Independent corners
                    dist = LGCalcDistanceIndependent(i.uv, _halfSize.xy, _rect2props, _r);
                    cornerAlpha = LGAntialiasedCutoff(dist);
                    shapeSize = _halfSize.xy * 2.0;
                } else if (_CornerMode > 0.5) {
                    // Uniform rounded corners
                    dist = LGCalcDistanceUniform(i.uv, _WidthHeightRadius.xy, _WidthHeightRadius.z);
                    cornerAlpha = LGAntialiasedCutoff(dist);
                } else {
                    // None (flat rectangle SDF)
                    float2 d = abs((i.uv - 0.5) * shapeSize) - (shapeSize * 0.5);
                ...
                // 2. Normal for bevel and meniscus
                float2 normal = LGGetSDFNormal(dist, i.uv, shapeSize);

                // 3. Screen coordinates
                float2 screenUV = LGGetScreenUV(i.screenPos, _FlipScreenY);

                // 4. Normal map and procedural waves
                ...
                // 6. Glass background sampling with blur amount control & chromatic dispersion
                half3 sceneBlur = LGSampleGlassBackground(_ScreenBlurTexture, _CameraOpaqueTexture, screenUV, totalDisp, _ChromaticAberration, _BlurAmount, _ScreenBlurTexture_TexelSize);
                ...
                // Output alpha: 1.0 inside the panel (showing blurred background), clipped by corner SDF
