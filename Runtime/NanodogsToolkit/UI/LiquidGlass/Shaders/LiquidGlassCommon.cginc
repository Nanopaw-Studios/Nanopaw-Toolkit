...
// Sample glass background with blur amount control, sharp scene blending, and boosted diffusion
inline half3 LGSampleGlassBackground(
    sampler2D blurTex,
    sampler2D sharpTex,
    float2 baseUV,
    float2 displacement,
    ...
    float4 texelSize)
{
    half3 blurCol = LGSampleChromaticAberration(blurTex, baseUV, displacement, dispersion);

    // If blurAmount is boosted (> 1.0), apply secondary 4-tap diffusion
    if (blurAmount > 1.01) {
        float2 tx = (texelSize.z > 0.0) ? texelSize.xy : float2(0.00052, 0.00092);
        float2 extraOffset = (blurAmount - 1.0) * 3.5 * tx;
        half3 extra = LGSampleChromaticAberration(blurTex, baseUV + float2(-extraOffset.x,  extraOffset.y), displacement, dispersion)
                    + LGSampleChromaticAberration(blurTex, baseUV + float2( extraOffset.x,  extraOffset.y), displacement, dispersion)
                    + LGSampleChromaticAberration(blurTex, baseUV + float2(-extraOffset.x, -extraOffset.y), displacement, dispersion)
                    + LGSampleChromaticAberration(blurTex, baseUV + float2( extraOffset.x, -extraOffset.y), displacement, dispersion);
        blurCol = lerp(blurCol, extra * 0.25, saturate(blurAmount - 1.0));
    }
    // If blurAmount is less than 1.0, blend between sharp background and blurred background
    else if (blurAmount < 0.99) {
        half3 sharpCol = LGSampleChromaticAberration(sharpTex, baseUV, displacement, dispersion);
        if (dot(sharpCol, sharpCol) > 0.0001) {
            blurCol = lerp(sharpCol, blurCol, saturate(blurAmount));
        }
    }
