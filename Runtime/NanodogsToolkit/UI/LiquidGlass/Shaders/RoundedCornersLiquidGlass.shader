Shader "UI/RoundedCorners/RoundedCornersLiquidGlass" {
    Properties {
        ...
        // Definition in Properties section is required for Mask to work properly
        _WidthHeightRadius ("WidthHeightRadius", Vector) = (100, 100, 20, 0)
    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag_lg(v2f_lg i) : SV_Target {
                // 1. Calculate Signed Distance Field for uniform rounded corners
                float2 rectSize = _WidthHeightRadius.xy;
                float radius = _WidthHeightRadius.z;
                float dist = LGCalcDistanceUniform(i.uv, rectSize, radius);
                ...
                float2 normal = LGGetSDFNormal(dist, i.uv, rectSize);
                ...
                // Fallback preview when texture is unrendered (e.g. Prefab mode / scene view before play)
                ...
                // Final alpha: UI image alpha * corner antialiasing
    ...
    FallBack "UI/RoundedCorners/RoundedCorners"
}
